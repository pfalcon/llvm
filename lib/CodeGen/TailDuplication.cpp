...
TailDuplicatePass::shouldTailDuplicate(const MachineFunction &MF,
  ...
  for (MachineBasicBlock::const_iterator I = TailBB.begin(); I != TailBB.end();
    ...
    // This should be covered by isNotDuplicable(), alas, it's not.
    // This in particular checks for inline asm with side effects.
// The problem here is that almost all asm (even asm("nop")) generated by
// Clang is marked as sideeffect. So, disable this check for now, fix Clang.
